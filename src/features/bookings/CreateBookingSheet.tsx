...
import { AddDogSheet } from '@/features/dogs/AddDogSheet';
      ...
      <div className="relative">
        ...
        {isOpen && (
          <div
            ...
            role="listbox"
            aria-labelledby="dog-label"
            className="absolute top-full left-0 right-0 mt-1 bg-white border border-pebble rounded-xl shadow-lg z-10 max-h-60 overflow-auto"
          >
            <button
              type="button"
              role="option"
              aria-selected={false}
              className="w-full flex items-center gap-3 p-3 border-b border-pebble/40 hover:bg-cream transition-colors"
              onClick={() => {
                setIsOpen(false);
                onAddNew?.();
              }}
            >
              <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-sage-light text-sage text-base font-black leading-none">
                +
              </span>
              <span className="text-sm font-semibold text-bark">Add New Dog</span>
            </button>
            {dogs.length === 0 ? (
              <div className="p-3 text-sm text-bark-light text-center">
                No dogs yet — add one now.
              </div>
            ) : (
              dogs.map((dog) => (
                <button
...
export function CreateBookingSheet({
  ...
  const handleAddDogSuccess = useCallback(
    (dog?: Dog) => {
      if (!dog?.id) return;
      setValue('dogId', dog.id, { shouldValidate: true, shouldDirty: true });
    },
    [setValue],
  );
